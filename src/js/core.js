...
const photoBooth = (function () {
    ...
    api.renderPic = function (filename, files, customQrlink) {
        ...
        $('<button>')
            ...
            .css('float', 'right')
            .appendTo(body);
        photoboothTools.console.logDev('api/qrcode.php?filename=' + filename + '&link=' + (customQrlink ? customQrlink : ''));
        $('<img src="api/qrcode.php?filename=' + filename + '&link=' + (customQrlink ? customQrlink : '') + '" alt="qr code"/>')
            .on('load', function () {
                $('<p>')
